...
|-------|-----------|----------|
| **22 mrt 2026** | **Scoreboard ↔ Mat koppeling + Reverb fix.** Poule sortering fix (gewichtsranges `20-21.8kg` sorteerden als `2021.8`). Scoreboard app kan nu authenticeren met mat URL+pincode (1 toegang per tafel voor browser+app). Reverb mat listener bug gefixt (subscribede op matnummer i.p.v. database ID). Staging volledig gesynchroniseerd via rsync (was out-of-sync met git). Scoreboard API config aangepast naar `env()` voor Reverb host/port. | smallwork.md |
