<?php $__env->startSection('title', __('Poules')); ?>
...
<div class="mb-4 p-4 bg-gray-100 border-l-4 border-gray-500 rounded no-print">
    <div class="flex items-center gap-3">
        ...
        <div>
            <p class="font-bold text-gray-800"><?php echo e(__('Voorbereiding afgesloten')); ?></p>
            <p class="text-sm text-gray-600"><?php echo __('De wedstrijddag is gestart. Wijzigingen aan poules kunnen alleen via <strong>Wedstrijddag > Poules</strong>.'); ?></p>
        </div>
    </div>
</div>
<?php endif; ?>
...
<?php if($heeftCategorieProbleem && !$isLocked): ?>
<div class="mb-4 no-print">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <div>
                    ...
                    <p class="text-sm text-red-700"><?php echo e(__('Pas de categorie-instellingen aan voordat je poules genereert.')); ?></p>
                </div>
            </div>
            <a href="<?php echo e(route('toernooi.edit', $toernooi->routeParams())); ?>?tab=toernooi" class="px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700 text-sm font-medium">
    ...
    <?php endif; ?>
    <?php if($overlapWarning): ?>
    <div class="p-3 bg-orange-100 border-l-4 border-orange-500 rounded">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
                <span class="text-xl">⚠️</span>
                <div>
                    <p class="font-bold text-orange-800"><?php echo e($overlapWarning); ?></p>
                    <p class="text-sm text-orange-700"><?php echo e(__('Categorieën mogen niet overlappen.')); ?></p>
                </div>
            </div>
            <a href="<?php echo e(route('toernooi.edit', $toernooi->routeParams())); ?>?tab=toernooi" class="px-3 py-1 bg-orange-600 text-white rounded hover:bg-orange-700 text-sm font-medium">
                <?php echo e(__('Naar instellingen')); ?>
...
<div id="inschrijving-popup" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden items-center justify-center no-print">
    <div class="bg-white rounded-lg shadow-2xl p-6 max-w-md mx-4 animate-pulse-warning">
        <div class="text-center">
            ...
            <h2 class="text-2xl font-bold text-orange-600 mt-4"><?php echo e(__('Inschrijving is gesloten!')); ?></h2>
            <p class="text-gray-600 mt-3"><?php echo e(__('De weegkaarten zijn al geprint met bloknummers. Wijzigingen hier kunnen problemen veroorzaken.')); ?></p>
            <p class="text-gray-700 font-medium mt-4"><?php echo e(__('Gebruik op de wedstrijddag:')); ?></p>
            <div class="flex flex-col gap-2 mt-3">
                <a href="<?php echo e(route('toernooi.wedstrijddag.poules', $toernooi->routeParams())); ?>" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg">
                    <?php echo e(__('Wedstrijddag Poules')); ?>

                </a>
                <a href="<?php echo e(route('toernooi.blok.zaaloverzicht', $toernooi->routeParams())); ?>" class="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-lg">
                    <?php echo e(__('Zaaloverzicht')); ?>

                </a>
            </div>
            <button onclick="sluitInschrijvingPopup()" class="mt-4 text-gray-500 hover:text-gray-700 text-sm underline">
                <?php echo e(__('Ik begrijp het, toch doorgaan')); ?>
...
<div id="poule-statistieken" class="bg-white rounded-lg shadow p-4 mb-6 no-print">
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
        <div>
            <div class="text-2xl font-bold text-blue-600" id="stat-poules"><?php echo e($poules->count()); ?></div>
            <div class="text-sm text-gray-600"><?php echo e(__('Poules')); ?></div>
        </div>
        <div>
            <div class="text-2xl font-bold text-green-600" id="stat-wedstrijden"><?php echo e($poules->sum('aantal_wedstrijden')); ?></div>
            <div class="text-sm text-gray-600"><?php echo e(__('Wedstrijden')); ?></div>
        </div>
        <div>
            <div class="text-2xl font-bold text-purple-600" id="stat-judokas"><?php echo e($poules->sum('judokas_count')); ?></div>
            <div class="text-sm text-gray-600"><?php echo e(__('Judoka\'s')); ?></div>
        </div>
        <div>
            <div class="text-2xl font-bold text-orange-600" id="stat-problematisch"><?php echo e($poules->filter(fn($p) => $isProblematischeGrootte($p->judokas_count) && $p->type !== 'eliminatie' && $p->type !== 'kruisfinale')->count()); ?></div>
            <div class="text-sm text-gray-600"><?php echo e(__('Problemen')); ?></div>
        </div>
    </div>
</div>

<div class="flex justify-between items-center mb-6 no-print">
    <h1 class="text-3xl font-bold text-gray-800"><?php echo e(__('Poules')); ?> (<span id="poule-count"><?php echo e($poules->count()); ?></span>)</h1>
    <?php if(!$isLocked): ?>
    <div class="flex items-center space-x-4">
        <span class="text-sm text-gray-500"><?php echo e(__('Sleep judoka\'s tussen poules')); ?></span>
        <?php if($heeftCategorieProbleem): ?>
        <span class="bg-gray-400 text-white font-bold py-2 px-4 rounded cursor-not-allowed opacity-60" title="<?php echo e(__('Los eerst categorie-problemen op')); ?>">
            <?php echo e(__('(her)Verdelen')); ?>

        </span>
        <?php else: ?>
        <form action="<?php echo e(route('toernooi.poule.genereer', $toernooi->routeParams())); ?>" method="POST" class="inline"
              data-loading="<?php echo e(__('Poule-indeling genereren...')); ?>"
              onsubmit="return <?php echo e($poules->count()); ?> === 0 || confirm('<?php echo e(__('WAARSCHUWING: Dit verwijdert ALLE huidige poules inclusief handmatige wijzigingen en maakt een nieuwe indeling. Weet je het zeker?')); ?>')">
            <?php echo csrf_field(); ?>
...
<?php if($problematischePoules->count() > 0): ?>
<div class="bg-red-50 border border-red-300 rounded-lg p-4 mb-6">
    <h3 class="font-bold text-red-800 mb-2"><?php echo e(__('Problematische poules')); ?> (<span id="problematische-count"><?php echo e($problematischePoules->count()); ?></span>)</h3>
    <p class="text-red-700 text-sm mb-3"><?php echo e(__('Poules met verkeerde grootte of te groot gewichtsverschil. Klik om naar de poule te gaan:')); ?></p>
    <div id="problematische-links" class="flex flex-wrap gap-2">
        <?php $__currentLoopData = $problematischePoules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <?php
        ...
        ?>
        <a href="#poule-<?php echo e($p->id); ?>" data-probleem-poule="<?php echo e($p->id); ?>" class="inline-flex items-center px-3 py-1 <?php echo e($chipClass); ?> rounded-full text-sm cursor-pointer transition-colors">
            #<?php echo e($p->nummer); ?> <?php echo e($p->getDisplayTitel()); ?> (<?php echo e($probleem); ?>)
        </a>
...
<?php $__empty_1 = true; $__currentLoopData = $poulesPerKlasse; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $categorieKey => $klassePoules): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
<?php
...
<div class="mb-8 w-full" x-data="{ open: true }">
    <div class="flex justify-between items-center bg-blue-800 text-white px-4 py-3 rounded-t-lg">
        <button @click="open = !open" class="flex-1 flex justify-between items-center hover:bg-blue-700 -m-3 p-3 rounded-tl-lg">
            <span class="text-lg font-bold"><?php echo e($categorieLabel); ?> (<?php echo e($klassePoules->count()); ?> poules, <?php echo e($klassePoules->sum('judokas_count')); ?> judoka's)</span>
            <svg :class="{ 'rotate-180': open }" class="w-5 h-5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
            </svg>
        </button>
        <button onclick="openNieuwePouleModal('<?php echo e($categorieKey); ?>', '<?php echo e($categorieLabel); ?>', <?php echo e($heeftVasteGewichten ? 'true' : 'false'); ?>)"
                class="ml-3 bg-white text-blue-800 hover:bg-blue-100 text-sm font-bold py-1.5 px-3 rounded">
            + <?php echo e(__('Nieuwe poule')); ?>
    ...
    <div x-show="open" x-collapse class="bg-gray-50 rounded-b-lg shadow p-4">
        ...
        ?>
        <?php $__currentLoopData = $poulesPerGewichtBlok; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $gewichtBlok => $gewichtPoules): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="mb-4 last:mb-0">
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            <?php $__currentLoopData = $gewichtPoules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $poule): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
            ...
            ?>
            <div id="poule-<?php echo e($poule->id); ?>" class="bg-white rounded-lg shadow flex flex-col <?php echo e($isEliminatie ? 'border-2 border-orange-400 col-span-full' : ''); ?> <?php echo e($isProbleem ? 'border-2 border-red-300' : ''); ?> <?php echo e($isKruisfinale ? 'border-2 border-purple-300' : ''); ?> <?php echo e($heeftGewichtWaarschuwing && !$isProbleem ? 'border-2 border-orange-300' : ''); ?>" data-poule-id="<?php echo e($poule->id); ?>" data-poule-nummer="<?php echo e($poule->nummer); ?>" data-poule-leeftijdsklasse="<?php echo e($poule->leeftijdsklasse); ?>" data-poule-gewichtsklasse="<?php echo e($poule->gewichtsklasse); ?>" data-poule-is-kruisfinale="<?php echo e($isKruisfinale ? '1' : '0'); ?>" data-poule-is-eliminatie="<?php echo e($isEliminatie ? '1' : '0'); ?>">
                <!-- Poule header -->
                <div class="px-3 py-2 border-b <?php echo e($isEliminatie ? 'bg-orange-100' : ($isKruisfinale ? 'bg-purple-100' : ($isProbleem ? 'bg-red-100' : 'bg-blue-100'))); ?>">
                    <div class="flex justify-between items-center">
                        <div class="font-bold text-sm <?php echo e($isEliminatie ? 'text-orange-800' : ($isKruisfinale ? 'text-purple-800' : ($isProbleem ? 'text-red-800' : 'text-blue-800'))); ?>">
                            <?php if($isEliminatie): ?>
                                <span data-poule-titel="<?php echo e($poule->id); ?>">#<?php echo e($poule->nummer); ?> ⚔️ <?php echo e($poule->getDisplayTitel()); ?></span> <span class="font-normal">(<?php echo e(__('Eliminatie')); ?>)</span>
                            <?php elseif($isKruisfinale): ?>
                                #<?php echo e($poule->nummer); ?> <?php echo e(__('Kruisfinale')); ?> <?php echo e($poule->gewichtsklasse); ?> kg
                            <?php else: ?>
                                <span class="text-gray-900" data-poule-titel="<?php echo e($poule->id); ?>">#<?php echo e($poule->nummer); ?> <?php echo e($poule->getDisplayTitel()); ?></span>
                                <?php if($heeftGewichtWaarschuwing): ?>
                                <span class="ml-1 text-orange-600" title="<?php echo e(__('Gewichtsverschil te groot: :verschilkg (max :maxkg)', ['verschil' => round($gewichtVerschil, 1), 'max' => $maxKgVerschil])); ?>">⚠️</span>
                                <?php endif; ?>
                            <?php endif; ?>
                        </div>
                        <div class="flex items-center gap-2">
                            
                            <?php if($isEliminatie || $isKruisfinale): ?>
                            <div class="relative" x-data="{ open: false }">
                                <button @click="open = !open" class="bg-gray-500 hover:bg-gray-600 text-white text-xs px-2 py-1 rounded">
                                    <?php echo e(__('Omzetten')); ?> ▾
                                </button>
                                <div x-show="open" @click.away="open = false" class="absolute right-0 mt-1 bg-white border rounded-lg shadow-lg z-10 min-w-[180px]">
                                    <?php if($isEliminatie): ?>
                                    <button onclick="zetOmNaarPoules(<?php echo e($poule->id); ?>, 'poules')" class="w-full text-left px-3 py-2 hover:bg-gray-100 text-sm">
                                        <?php echo e(__('Alleen poules')); ?>

                                    </button>
                                    <button onclick="zetOmNaarPoules(<?php echo e($poule->id); ?>, 'poules_kruisfinale')" class="w-full text-left px-3 py-2 hover:bg-gray-100 text-sm border-t">
                                        <?php echo e(__('Poules + kruisfinale')); ?>

                                    </button>
                                    <?php else: ?>
                                    <button onclick="zetOmNaar(<?php echo e($poule->id); ?>, 'poules')" class="w-full text-left px-3 py-2 hover:bg-gray-100 text-sm">
                                        <?php echo e(__('Alleen poules')); ?>

                                    </button>
                                    <button onclick="zetOmNaar(<?php echo e($poule->id); ?>, 'eliminatie')" class="w-full text-left px-3 py-2 hover:bg-gray-100 text-sm border-t">
                                        <?php echo e(__('Eliminatie')); ?>
                            ...
                            <?php endif; ?>
                            <?php if($poule->judokas_count === 0): ?>
                            <button onclick="verwijderPoule(<?php echo e($poule->id); ?>, '<?php echo e($poule->nummer); ?>')" class="delete-empty-btn text-red-500 hover:text-red-700 font-bold text-lg leading-none" title="<?php echo e(__('Verwijder poule')); ?>">&minus;</button>
                            <?php endif; ?>
                        </div>
                    </div>
                    <div class="flex justify-between items-center text-xs text-gray-500">
                        <?php if($poule->isKruisfinale()): ?>
                        <span class="flex items-center gap-1">
                            Top
                            <select onchange="updateKruisfinalesPlaatsen(<?php echo e($poule->id); ?>, this.value)" class="border rounded px-1 py-0.5 text-xs bg-white">
                                <?php for($i = 1; $i <= 3; $i++): ?>
                                <option value="<?php echo e($i); ?>" <?php echo e($poule->kruisfinale_plaatsen == $i ? 'selected' : ''); ?>><?php echo e($i); ?></option>
                                <?php endfor; ?>
                            </select>
                            <?php echo e(__('door')); ?> → <span data-poule-count="<?php echo e($poule->id); ?>"><?php echo e($poule->aantal_judokas); ?></span> <?php echo e(__('judoka\'s')); ?>

                        </span>
                        <?php else: ?>
                        <span><span data-poule-count="<?php echo e($poule->id); ?>"><?php echo e($poule->judokas_count ?: $poule->aantal_judokas); ?></span> <?php echo e(__('judoka\'s')); ?></span>
                        <?php endif; ?>
                        <span><span data-poule-wedstrijden="<?php echo e($poule->id); ?>"><?php echo e(($poule->judokas_count ?: $poule->aantal_judokas) < 2 && !$poule->isKruisfinale() ? '-' : $poule->aantal_wedstrijden); ?></span> <?php echo e(__('wedstrijden')); ?></span>
                    </div>
                </div>

                <!-- Judoka's in poule (sortable) -->
                <div class="<?php echo e($isEliminatie ? 'grid grid-cols-5 gap-1 p-2' : 'divide-y divide-gray-100'); ?> min-h-[60px] flex-1 sortable-poule" data-poule-id="<?php echo e($poule->id); ?>">
                    ...
                    ?>
                    <div class="px-3 py-2 hover:bg-blue-50 cursor-move text-sm judoka-item group <?php echo e($isEliminatie ? 'border border-gray-200 rounded' : ''); ?>"
                         data-judoka-id="<?php echo e($judoka->id); ?>"
                         data-poule-id="<?php echo e($poule->id); ?>"
                         data-judoka-naam="<?php echo e($judoka->naam); ?>"
                         data-judoka-leeftijd="<?php echo e($judoka->leeftijd); ?>"
                         data-judoka-gewicht="<?php echo e($judoka->gewicht ?? ''); ?>">
                        <div class="flex justify-between items-start">
                            <div class="flex-1 min-w-0">
                                <div class="font-medium text-gray-800 truncate"><?php echo e($judoka->naam); ?> <span class="text-gray-400 font-normal">(<?php echo e($judoka->leeftijd); ?>j)</span></div>
                                <div class="text-xs text-gray-500 truncate"><?php echo e($judoka->club?->naam ?? '-'); ?></div>
                            </div>
                            <div class="text-right text-xs">
                                <div class="<?php echo e($isGewogen ? 'text-green-600' : 'text-gray-600'); ?> font-medium"><?php echo e($toonGewicht ?? '-'); ?><?php if($isTeZwaar): ?> <span title="<?php echo e(__('Te zwaar voor :klasse', ['klasse' => $poule->gewichtsklasse])); ?>">⚠️</span><?php endif; ?></div>
                                <div class="text-gray-400"><?php echo e(\App\Enums\Band::toKleur($judoka->band)); ?></div>
                            </div>
                            <button
                                onclick="event.stopPropagation(); openZoekMatchFor(<?php echo e($judoka->id); ?>, this.closest('.judoka-item'))"
                    ...
                    <?php if($poule->judokas->isEmpty()): ?>
                    <div class="px-3 py-4 text-gray-400 text-sm italic text-center empty-placeholder <?php echo e($isEliminatie ? 'col-span-5' : ''); ?>"><?php echo e(__('Leeg')); ?></div>
                    <?php endif; ?>
                </div>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<div class="bg-white rounded-lg shadow p-8 text-center text-gray-500">
    <?php echo e(__('Nog geen poules. Genereer eerst de poule-indeling.')); ?>
...
<div id="nieuwe-poule-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        ...
        <form id="nieuwe-poule-form">
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1"><?php echo e(__('Leeftijdsklasse')); ?></label>
                <select id="leeftijdsklasse" class="w-full border rounded px-3 py-2" required>
                    ...
                    <option value="<?php echo e($key); ?>" data-label="<?php echo e($klasse['label']); ?>"><?php echo e($klasse['label']); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </div>
            <div class="mb-4" id="gewichtsklasse-container">
                <label class="block text-sm font-medium text-gray-700 mb-1"><?php echo e(__('Gewichtsklasse')); ?></label>
                <select id="gewichtsklasse" class="w-full border rounded px-3 py-2" disabled>
                    <option value=""><?php echo e(__('Selecteer eerst leeftijdsklasse')); ?></option>
                </select>
            </div>
...
<div id="judoka-context-menu" class="fixed hidden bg-white rounded-lg shadow-xl border border-gray-200 py-1 z-50 min-w-[160px]">
    <button onclick="openZoekMatch()" class="w-full px-4 py-2 text-left text-sm hover:bg-blue-50 flex items-center gap-2">
        <span>🔍</span> <?php echo e(__('Zoek match')); ?>
...
<div id="zoek-match-modal" class="fixed inset-0 bg-black bg-opacity-30 hidden z-50 pointer-events-none">
    <div id="zoek-match-modal-content" class="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col absolute pointer-events-auto" style="top: 50%; left: 50%; transform: translate(-50%, -50%);">
        ...
        <div class="p-4 overflow-y-auto flex-1" id="zoek-match-results">
            <p class="text-gray-500 text-center py-8"><?php echo e(__('Laden...')); ?></p>
        </div>
    </div>
</div>
...
<script>
// i18n constants
const __leeg = <?php echo json_encode(__('Leeg'), 15, 512) ?>;
const __bezigMetVerificatie = <?php echo json_encode(__('Bezig met verificatie...'), 15, 512) ?>;
const __laden = <?php echo json_encode(__('Laden...'), 15, 512) ?>;
const __geenPassendePoules = <?php echo json_encode(__('Geen passende poules gevonden'), 15, 512) ?>;
const __foutBijOphalenMatches = <?php echo json_encode(__('Fout bij ophalen matches'), 15, 512) ?>;
const __foutBijOphalen = <?php echo json_encode(__('Fout bij ophalen'), 15, 512) ?>;
const __verwijderPoule = <?php echo json_encode(__('Verwijder poule'), 15, 512) ?>;
const __selecteer = <?php echo json_encode(__('Selecteer...'), 15, 512) ?>;
const __selecteerEerstLeeftijdsklasse = <?php echo json_encode(__('Selecteer eerst leeftijdsklasse'), 15, 512) ?>;
const __variabel = <?php echo json_encode(__('Variabel'), 15, 512) ?>;
const __problematischePoules = <?php echo json_encode(__('Problematische poules'), 15, 512) ?>;
const __dezePoulesMinder3 = <?php echo json_encode(__('Deze poules hebben minder dan 3 judoka\'s. Klik om naar de poule te gaan:'), 15, 512) ?>;
const __foutBijOmzetten = <?php echo json_encode(__('Fout bij omzetten'), 15, 512) ?>;
const __foutBijAanmaken = <?php echo json_encode(__('Fout bij aanmaken'), 15, 512) ?>;
...
const __omgezetNaarPoules = <?php echo json_encode(__('Omgezet naar poules'), 15, 512) ?>;
const __omgezet = <?php echo json_encode(__('Omgezet'), 15, 512) ?>;
const __pouleVerwijderen = <?php echo json_encode(__('Poule #:nummer verwijderen?'), 15, 512) ?>;
const __eliminatieOmzettenPoules = <?php echo json_encode(__('Eliminatie omzetten naar alleen poules?'), 15, 512) ?>;
const __eliminatieOmzettenPoulesKruisfinale = <?php echo json_encode(__('Eliminatie omzetten naar poules + kruisfinale?'), 15, 512) ?>;
const __omzettenNaarEliminatie = <?php echo json_encode(__('Omzetten naar eliminatie?'), 15, 512) ?>;
const __omzettenNaarPoules = <?php echo json_encode(__('Omzetten naar poules?'), 15, 512) ?>;
const __verificatieProblemenGevonden = <?php echo json_encode(__('Verificatie: :aantal probleem(en) gevonden'), 15, 512) ?>;
const __verificatieGeslaagd = <?php echo json_encode(__('Verificatie geslaagd!'), 15, 512) ?>;
const __allePoulesCorrect = <?php echo json_encode(__('Alle :totaal poules zijn correct. :wedstrijden wedstrijden gepland.'), 15, 512) ?>;
const __poulesHerberekend = <?php echo json_encode(__(':aantal poules herberekend'), 15, 512) ?>;
const __paginaVernieuwen = <?php echo json_encode(__('Pagina vernieuwen'), 15, 512) ?>;
const __omWijzigingenTeZien = <?php echo json_encode(__('om wijzigingen te zien'), 15, 512) ?>;

const isLocked = <?php echo e($isLocked ? 'true' : 'false'); ?>;
const wegingTolerantie = <?php echo e($toernooi->weging_tolerantie ?? 0.5); ?>;
const __teZwaarVoor = <?php echo json_encode(__('Te zwaar voor :klasse'), 15, 512) ?>;
const __confirmUitschrijven = <?php echo json_encode(__(':naam afmelden en uit poule verwijderen?'), 15, 512) ?>;
const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
const verifieerUrl = '<?php echo e(route('toernooi.poule.verifieer', $toernooi->routeParams())); ?>';
const verplaatsUrl = '<?php echo e(route('toernooi.poule.verplaats-judoka-api', $toernooi->routeParams())); ?>';
const zoekMatchUrl = '<?php echo e(route('toernooi.poule.zoek-match', $toernooi->routeParamsWith(['judoka' => '__JUDOKA_ID__']))); ?>';
const nieuwePouleUrl = '<?php echo e(route('toernooi.poule.store', $toernooi->routeParams())); ?>';
...
const uitschrijvenUrl = '<?php echo e(route('toernooi.poule.uitschrijven', $toernooi->routeParamsWith(['judoka' => '__JUDOKA_ID__']))); ?>';
const zetOmNaarPoulesUrl = '<?php echo e(route('toernooi.wedstrijddag.zetOmNaarPoules', $toernooi->routeParams())); ?>';
const wijzigPouleTypeUrl = '<?php echo e(route('toernooi.wedstrijddag.wijzigPouleType', $toernooi->routeParams())); ?>';

// Gewichtsklassen per leeftijdsklasse
const gewichtsklassen = <?php echo json_encode($toernooi->getAlleGewichtsklassen(), 15, 512) ?>;

// Toegestane poule groottes uit instellingen
const toegestaneGroottes = <?php echo json_encode($toegestaneGroottes, 15, 512) ?>;
...
async function verwijderPoule(pouleId, pouleNummer) {
    ...
    try {
        ...
        if (data.success) {
            ...
            showToast(data.message || '<?php echo e(__('Fout bij verwijderen')); ?>', true);
        }
    } catch (error) {
        console.error('Error:', error);
        showToast('<?php echo e(__('Fout bij verwijderen')); ?>', true);
    }
}
    ...
    try {
        ...
        if (data.success) {
            ...
            if (pouleBody && pouleBody.children.length === 0) {
                pouleBody.innerHTML = '<div class="px-3 py-4 text-gray-400 text-sm italic text-center empty-placeholder"><?php echo e(__('Leeg')); ?></div>';
            }
            updateTotaalStats();
        } else {
            showToast(data.message || '<?php echo e(__('Fout bij uitschrijven')); ?>', true);
        }
    } catch (error) {
        console.error('Error:', error);
        showToast('<?php echo e(__('Fout bij uitschrijven')); ?>', true);
    }
}
...
async function updateKruisfinalesPlaatsen(pouleId, plaatsen) {
    try {
        ...
        if (data.success) {
            ...
            showToast(data.message || '<?php echo e(__('Fout bij opslaan')); ?>', true);
        }
    } catch (error) {
        console.error('Error:', error);
        showToast('<?php echo e(__('Fout bij opslaan')); ?>', true);
    }
}
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/judotoernooi/staging/resources/views/pages/poule/index.blade.php ENDPATH**/ ?>