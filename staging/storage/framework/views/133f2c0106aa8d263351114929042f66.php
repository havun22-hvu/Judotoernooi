<?php $__env->startSection('title', __('Blokverdeling')); ?>
...
<div class="flex justify-between items-center mb-4">
    <div class="flex items-center gap-4">
        <h1 class="text-3xl font-bold text-gray-800"><?php echo e(__('Blokverdeling')); ?></h1>
        <span class="text-sm text-gray-500"><?php echo e($blokken->count()); ?> <?php echo e(__('blokken')); ?> | <?php echo e($totaalWedstrijden); ?> <?php echo e(__('wed.')); ?> | <?php echo e(__('gem')); ?> <?php echo e($gemiddeldPerBlok); ?>/<?php echo e(__('blok')); ?></span>
        <?php if($nietVerdeeldCats->isNotEmpty()): ?>
        <span class="text-sm text-red-600 font-bold" id="niet-verdeeld-header"><?php echo e($nietVerdeeldCats->count()); ?> <?php echo e(__('cat. niet verdeeld')); ?></span>
        <?php endif; ?>
    </div>
    <div class="flex items-center gap-2">
        <?php if($heeftVariabeleCategorieen): ?>
        
        <form action="<?php echo e(route('toernooi.blok.genereer-variabele-verdeling', $toernooi->routeParams())); ?>" method="POST" class="inline">
            <?php echo csrf_field(); ?>
            <button type="submit" class="bg-orange-600 hover:bg-orange-700 text-white font-bold py-2 px-4 rounded">
                <?php echo e(__('Verdeel variabel')); ?>

            </button>
        </form>
        <?php else: ?>
        
        <form action="<?php echo e(route('toernooi.blok.genereer-verdeling', $toernooi->routeParams())); ?>" method="POST" class="inline flex items-center gap-2" id="bereken-form">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="balans" id="balans-input" value="<?php echo e(session('blok_balans', 50)); ?>">
            <div class="flex items-center gap-2 text-xs bg-gray-100 px-3 py-1.5 rounded">
                <span class="text-gray-600 whitespace-nowrap"><?php echo e(__('Verdeling')); ?></span>
                <input type="range" id="balans-slider-header" min="0" max="100" value="<?php echo e(session('blok_balans', 50)); ?>"
                ...
                <span class="text-gray-600 whitespace-nowrap"><?php echo e(__('Aansluiting')); ?></span>
            </div>
            <button type="submit" id="bereken-btn" class="bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded min-w-[160px]">
                <span id="bereken-tekst"><?php echo e(__('(her)Bereken')); ?></span>
                <span id="bereken-loading" style="display: none;">
                    ...
                    <?php echo e(__('Berekenen...')); ?>

                </span>
            </button>
        </form>
        <?php endif; ?>
        <form action="<?php echo e(route('toernooi.blok.zet-op-mat', $toernooi->routeParams())); ?>" method="POST" class="inline" id="zet-op-mat-form">
            <?php echo csrf_field(); ?>
            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded" title="<?php echo e(__('Verdeel poules over matten en ga naar zaaloverzicht')); ?>">
                <?php echo e(__('Verdeel over matten')); ?> →
            </button>
        </form>
    </div>
</div>
...
<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded mb-4 text-sm"><?php echo e(session('error')); ?></div>
<?php endif; ?>

<?php if($blokken->isEmpty()): ?>
<div class="bg-yellow-50 border border-yellow-300 rounded-lg p-6 text-center">
    <p class="text-yellow-800"><?php echo e(__('Nog geen blokken. Maak eerst blokken aan in toernooi instellingen.')); ?></p>
</div>
<?php else: ?>
...
<div class="flex gap-4">
    ...
    <div class="flex-1 space-y-3">
        ...
        <div class="bg-white rounded-lg shadow" id="sleepvak-container">
            <div class="bg-purple-700 text-white px-4 py-2 rounded-t-lg flex justify-between items-center cursor-pointer" onclick="document.getElementById('niet-verdeeld-content').classList.toggle('hidden')">
                <div class="flex items-center gap-3">
                    ...
                    <span class="font-bold"><?php echo e(__('Sleepvak')); ?></span>
                    <span class="text-purple-200" id="sleepvak-stats">(<?php echo e($nietVerdeeldCats->count()); ?> cat., <?php echo e($nietVerdeeldCats->sum('wedstrijden')); ?>w)</span>
                </div>
            </div>
            <div id="niet-verdeeld-content" class="p-2 blok-dropzone min-h-[40px]" data-blok="0">
                <?php if($nietVerdeeldCats->isEmpty()): ?>
                <span class="text-gray-400 text-xs italic dropzone-placeholder"><?php echo e(__('Alle categorieën zijn verdeeld')); ?></span>
                <?php else: ?>
                <div class="flex flex-wrap gap-x-4 gap-y-1">
                    <?php $__currentLoopData = $leeftijdVolgorde; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $leeftijd): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php $leeftijdCats = $nietVerdeeldPerLeeftijd->get($leeftijd, collect()); ?>
                        <?php if($leeftijdCats->isNotEmpty()): ?>
                        <div class="leeftijd-groep" data-leeftijd="<?php echo e($leeftijd); ?>">
                            <div class="font-semibold text-gray-700 text-xs mb-0.5">
                                <?php echo e($afkortingen[$leeftijd] ?? $leeftijd); ?>

                                <span class="text-gray-400 font-normal">(<?php echo e($leeftijdCats->sum('wedstrijden')); ?>w)</span>
                            </div>
                            <div class="leeftijd-content flex flex-wrap gap-0.5">
                                <?php $__currentLoopData = $leeftijdCats->sortBy(fn($c) => (int)preg_replace('/[^0-9]/', '', $c['gewicht']) + (str_starts_with($c['gewicht'], '+') ? 500 : 0)); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php echo $__env->make('pages.blok._category_chip', ['cat' => $cat, 'inSleepvak' => true], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        ...
        ?>
        <div class="bg-white rounded-lg shadow blok-container" data-blok="<?php echo e($blok->nummer); ?>" data-blok-id="<?php echo e($blok->id); ?>" data-gewenst="<?php echo e($gewenstWedstrijden); ?>">
            <div class="bg-gray-800 text-white px-4 py-2 rounded-t-lg flex justify-between items-center">
                <div class="flex items-center gap-4">
                    <span class="font-bold"><?php echo e(__('Blok')); ?> <?php echo e($blok->nummer); ?></span>
                    <?php if($blok->blok_label): ?>
                    <span class="text-orange-300 text-sm font-medium"><?php echo e($blok->blok_label); ?></span>
                    <?php endif; ?>
                    <div class="flex items-center gap-2 text-sm">
                        <span class="text-gray-400"><?php echo e(__('Gewenst:')); ?></span>
                        <input type="number" class="gewenst-input w-16 px-2 py-1 rounded text-gray-800 text-center text-sm"
                               value="<?php echo e($blok->gewenst_wedstrijden ?? ''); ?>"
                               placeholder="<?php echo e($gemiddeldPerBlok); ?>"
                               data-blok-id="<?php echo e($blok->id); ?>"
                               onchange="updateGewenst(this)">
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <span class="blok-stats text-sm"><?php echo e(__('Actueel:')); ?> <strong class="blok-actueel"><?php echo e($blokWedstrijden); ?></strong> <?php echo e(__('wed.')); ?></span>
                    <span class="afwijking-badge text-xs px-2 py-0.5 rounded <?php echo e(abs($afwijkingPct) <= 10 ? 'bg-green-500' : (abs($afwijkingPct) <= 25 ? 'bg-yellow-500' : 'bg-red-500')); ?>">
                        <?php echo e($afwijkingPct >= 0 ? '+' : ''); ?><?php echo e($afwijkingPct); ?>%
                    </span>
                    <?php if($blok->weging_gesloten): ?><span class="text-xs bg-red-500 px-2 py-0.5 rounded"><?php echo e(__('Gesloten')); ?></span><?php endif; ?>
                </div>
            </div>
            <div class="p-3 min-h-[50px] blok-dropzone flex flex-wrap gap-1.5 items-start content-start" data-blok="<?php echo e($blok->nummer); ?>">
                <?php $__currentLoopData = $blokCatsSorted; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php echo $__env->make('pages.blok._category_chip', ['cat' => $cat, 'inSleepvak' => false], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                <?php if($blokCats->isEmpty()): ?><span class="text-gray-400 text-xs italic dropzone-placeholder"><?php echo e(__('Sleep categorieën hierheen')); ?></span><?php endif; ?>
            </div>
        </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ...
        <div class="bg-white rounded-lg shadow p-3">
            ...
            <div class="flex justify-end gap-3 mb-2">
                <button type="button" onclick="pasVariantToe()" class="bg-green-600 hover:bg-green-700 text-white text-xs font-bold px-3 py-1 rounded">
                    ✓ <?php echo e(__('Toepassen')); ?>

                </button>
                <a href="<?php echo e(route('toernooi.blok.index', $toernooi->routeParams())); ?>" class="text-gray-400 hover:text-gray-600 text-xs">✕ <?php echo e(__('Annuleer')); ?></a>
            </div>
            <?php endif; ?>
            ...
            <div class="flex flex-wrap gap-2 mb-2">
                <?php if($toonVarianten): ?>
                    <?php $__currentLoopData = $varianten; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $variant): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php
                    ...
                    ?>
                    <button type="button" onclick="toonVariant(<?php echo e($idx); ?>)"
                            class="variant-btn px-3 py-2 rounded border text-sm transition-all <?php echo e($idx === 0 ? 'border-blue-500 bg-blue-50 ring-2 ring-blue-200' : 'border-gray-200 bg-white hover:bg-gray-50'); ?>"
                            data-idx="<?php echo e($idx); ?>"
                            data-origineel-score="<?php echo e($totaal); ?>"
                            title="Score: <?php echo e($totaal); ?> (lager=beter)&#10;Verdeling: <?php echo e($scores['verdeling_score'] ?? 0); ?> (Σ%afwijking)&#10;Aansluiting: <?php echo e($scores['aansluiting_score'] ?? 0); ?> (0/10/20/30 per overgang)&#10;Weging: <?php echo e($scores['gewichten']['verdeling'] ?? 50); ?>% / <?php echo e($scores['gewichten']['aansluiting'] ?? 50); ?>%">
                        <span class="font-bold variant-nummer">#<?php echo e($idx + 1); ?></span>
                        <span class="variant-score font-bold <?php echo e($scoreKleur); ?>"><?php echo e($totaal); ?></span>
                        <span class="variant-detail text-gray-400 text-xs">(V<?php echo e($scores['verdeling_score'] ?? 0); ?>+A<?php echo e($scores['aansluiting_score'] ?? 0); ?>)</span>
                    </button>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                <?php else: ?>
                    <?php for($i = 1; $i <= 5; $i++): ?>
                    <button type="button" disabled
                        ...
                        <span class="font-bold">#<?php echo e($i); ?></span>
                        <span>--</span>
                    </button>
                    <?php endfor; ?>
                <?php endif; ?>
            ...
            <div class="text-sm text-gray-500">
                <?php if($toonVarianten && !empty($blokStats)): ?>
                    <span class="font-medium"><?php echo e(number_format($blokStats['pogingen'] ?? 0)); ?></span>
                    <span class="text-gray-400 text-xs">(<?php echo e($blokStats['tijd_sec'] ?? 0); ?>s, <?php echo e(number_format($blokStats['per_seconde'] ?? 0)); ?>/s)</span>
                    <span class="text-gray-400 mx-1">→</span>
                    <span><?php echo e($blokStats['geldige_varianten'] ?? 0); ?> <?php echo e(__('geldig')); ?></span>
                    <span class="text-gray-400 mx-1">→</span>
                    <span class="font-bold text-blue-600">top <?php echo e($blokStats['getoond'] ?? 0); ?></span>
                    <span id="live-score-display" class="ml-3 hidden"></span>
                <?php else: ?>
                    <?php echo e(__('Varianten (klik Bereken)')); ?>
    ...
    <div class="w-80 flex-shrink-0">
        <div class="bg-white rounded-lg shadow sticky top-4">
            <div class="bg-gray-700 text-white px-3 py-2 rounded-t-lg flex justify-between items-center text-sm">
                <span class="font-bold"><?php echo e(__('Overzicht')); ?></span>
                <span class="text-gray-300"><?php echo e(__('Blok')); ?></span>
            </div>
            <div class="p-2 max-h-[calc(100vh-200px)] overflow-y-auto text-xs" id="overzicht-panel">
                ...
                <?php $__currentLoopData = $leeftijdVolgorde; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $leeftijd): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <?php if($leeftijdCats->isNotEmpty()): ?>
                    <div class="mb-2">
                        <div class="font-bold text-gray-700 border-b border-gray-200 pb-0.5 mb-1"><?php echo e($afkortingen[$leeftijd] ?? $leeftijd); ?></div>
                        
                        <?php $__currentLoopData = $leeftijdCats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php
                        ...
                        <div class="flex justify-between items-center py-0.5 hover:bg-gray-50 border-b border-gray-100">
                            <div class="flex-1 min-w-0">
                                <span class="font-bold text-gray-600">#<?php echo e($cat['nummer']); ?></span>
                                <span class="text-gray-700 ml-1"><?php echo e($titelKort); ?></span>
                            </div>
                            <div class="flex items-center gap-1 flex-shrink-0">
                                <span class="text-gray-400">(<?php echo e($cat['wedstrijden']); ?>w)</span>
                                <?php if($cat['blok']): ?>
                                    <?php if($cat['vast']): ?>
                                    <span class="bg-green-100 text-green-800 px-1.5 py-0.5 rounded font-bold blok-badge" data-key="<?php echo e($dataKey); ?>">●<?php echo e($cat['blok']); ?></span>
                                    <?php else: ?>
                                    <span class="bg-blue-100 text-blue-800 px-1.5 py-0.5 rounded font-bold blok-badge" data-key="<?php echo e($dataKey); ?>"><?php echo e($cat['blok']); ?></span>
                                    <?php endif; ?>
                                <?php else: ?>
                                <span class="bg-red-100 text-red-600 px-1.5 py-0.5 rounded blok-badge" data-key="<?php echo e($dataKey); ?>">-</span>
                                <?php endif; ?>
                            </div>
                        </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<div class="text-xs text-green-600 mt-1">✓ <?php echo e(session('success')); ?></div>
<?php endif; ?>

<!-- Varianten data voor JavaScript -->
<?php if($toonVarianten): ?>
<script>
const variantenData = <?php echo json_encode($varianten, 15, 512) ?>;
const afkortingen = <?php echo json_encode($afkortingen, 15, 512) ?>;
</script>
<?php endif; ?>
...
function updateGewenst(input) {
    ...
    fetch('<?php echo e(route('toernooi.blok.update-gewenst', $toernooi->routeParams())); ?>', {
        method: 'POST',
...
function pasVariantToe() {
    ...
    fetch('<?php echo e(route('toernooi.blok.kies-variant', $toernooi->routeParams())); ?>', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' },
...
const leeftijdVolgorde = <?php echo json_encode($leeftijdVolgorde, 15, 512) ?>;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.blok-dropzone').forEach(zone => {
        ...
        zone.addEventListener('drop', function(e) {
            ...
            fetch('<?php echo e(route('toernooi.blok.verplaats-categorie', $toernooi->routeParams())); ?>', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' },
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/judotoernooi/staging/resources/views/pages/blok/index.blade.php ENDPATH**/ ?>