...
<script>
    ...
    const __t = {
        sessionExpired: <?php echo json_encode(__('Sessie verlopen. De pagina wordt herladen.'), 15, 512) ?>,
        serverErrorPasswordCheck: <?php echo json_encode(__('Serverfout bij wachtwoord check (HTTP :status). Probeer de pagina te herladen.'), 15, 512) ?>,
        couldNotCheckPassword: <?php echo json_encode(__('Kon wachtwoord niet controleren (netwerkfout). Probeer de pagina te herladen.'), 15, 512) ?>,
        incorrectPassword: <?php echo json_encode(__('Onjuist wachtwoord! Actie geannuleerd.'), 15, 512) ?>,
        deleteError: <?php echo json_encode(__('Fout bij verwijderen'), 15, 512) ?>,
        resetError: <?php echo json_encode(__('Fout bij terugzetten'), 15, 512) ?>,
        resetFailed: <?php echo json_encode(__('Terugzetten mislukt'), 15, 512) ?>,
        serverErrorNoJson: <?php echo json_encode(__('Server fout (geen JSON response)'), 15, 512) ?>,
        reloadAndRetry: <?php echo json_encode(__('Herlaad de pagina en probeer opnieuw.'), 15, 512) ?>,
        blocked: <?php echo json_encode(__('GEBLOKKEERD'), 15, 512) ?>,
        autoCorrections: <?php echo json_encode(__('Automatische correcties uitgevoerd'), 15, 512) ?>,
        networkError: <?php echo json_encode(__('Netwerkfout'), 15, 512) ?>,
        checkConnection: <?php echo json_encode(__('Controleer je internetverbinding. De pagina wordt herladen.'), 15, 512) ?>,
        placementError: <?php echo json_encode(__('Fout bij plaatsen'), 15, 512) ?>,
        noFinaleFound: <?php echo json_encode(__('Geen finale gevonden!'), 15, 512) ?>,
        wrongMatch: <?php echo json_encode(__(':naam komt niet uit de juiste wedstrijd!'), 15, 512) ?>,
        onlyParticipants: <?php echo json_encode(__('Alleen deelnemers uit deze wedstrijd kunnen op :medaille geplaatst worden.'), 15, 512) ?>,
        bronze: <?php echo json_encode(__('brons'), 15, 512) ?>,
        goldSilver: <?php echo json_encode(__('goud/zilver'), 15, 512) ?>,
        matchModifiedByOther: <?php echo json_encode(__('Deze wedstrijd is gewijzigd door een ander apparaat.'), 15, 512) ?>,
        errorLabel: <?php echo json_encode(__('Fout'), 15, 512) ?>,
        unknownError: <?php echo json_encode(__('Onbekende fout'), 15, 512) ?>,
        medalPlacementError: <?php echo json_encode(__('Fout bij medaille plaatsing'), 15, 512) ?>,
        deleteErrorShort: <?php echo json_encode(__('Fout bij verwijderen'), 15, 512) ?>,
        finishGreenFirst: <?php echo json_encode(__('Eerst de groene wedstrijd afmaken!'), 15, 512) ?>,
        bracketLoadError: <?php echo json_encode(__('Fout bij laden bracket'), 15, 512) ?>,
        bracketLoadErrorConsole: <?php echo json_encode(__('Fout bij laden bracket HTML'), 15, 512) ?>,
        advanceByesError: <?php echo json_encode(__('Fout bij advance byes'), 15, 512) ?>,
        loadError: <?php echo json_encode(__('Fout bij laden'), 15, 512) ?>,
        loadMatchesError: <?php echo json_encode(__('Fout bij laden wedstrijden'), 15, 512) ?>,
        serverError: <?php echo json_encode(__('Server fout (:status). Probeer de pagina te herladen.'), 15, 512) ?>,
        allRounds: <?php echo json_encode(__('Alle rondes'), 15, 512) ?>,
        fromRound: <?php echo json_encode(__('Vanaf ronde :nummer'), 15, 512) ?>,
        markDoneError: <?php echo json_encode(__('Fout bij markeren'), 15, 512) ?>,
        barrageError: <?php echo json_encode(__('Fout bij maken barrage'), 15, 512) ?>,
        noBarrageNeeded: <?php echo json_encode(__('Geen barrage nodig'), 15, 512) ?>,
        barrageCreated: <?php echo json_encode(__('Barrage poule #:nummer aangemaakt op dezelfde mat!'), 15, 512) ?>,
        saveSelectionError: <?php echo json_encode(__('Fout bij opslaan selectie'), 15, 512) ?>,
        selectMatFirst: <?php echo json_encode(__('Selecteer eerst een mat.'), 15, 512) ?>,
        allSlotsTaken: <?php echo json_encode(__('Alle slots zijn bezet (groen + geel + blauw). Klik op een gekleurde wedstrijd om te deselecteren.'), 15, 512) ?>,
        stopGreenMatch: <?php echo json_encode(__('Groene wedstrijd stoppen? Geel wordt groen, blauw wordt geel.'), 512) ?>,
        noMatSelected: <?php echo json_encode(__('Geen mat geselecteerd. Ververs de pagina.'), 15, 512) ?>,
        maxSeedingSwaps: <?php echo json_encode(__('Maximum aantal seeding swaps bereikt (:max). De bracket is nu vastgezet.'), 15, 512) ?>,
        passwordOrPinPlaceholder: <?php echo json_encode(__('Wachtwoord of pincode'), 15, 512) ?>,
        cancelButton: <?php echo json_encode(__('Annuleren'), 15, 512) ?>,
        played: <?php echo json_encode(__('Gespeeld'), 15, 512) ?>,
        playingNow: <?php echo json_encode(__('Speelt nu - klik om te stoppen (geel wordt groen, blauw wordt geel)'), 512) ?>,
        standingBy: <?php echo json_encode(__('Staat klaar - klik om te deselecteren (blauw wordt geel)'), 15, 512) ?>,
        gettingReady: <?php echo json_encode(__('Gereed maken - klik om te deselecteren'), 15, 512) ?>,
        clickToSelect: <?php echo json_encode(__('Klik om te selecteren'), 15, 512) ?>,
        barrageCreateFor: <?php echo json_encode(__('Barrage maken voor: :namen?'), 15, 512) ?>,
        wantToMoveToSwap: <?php echo json_encode(__('Je wilt :naam naar de swap verplaatsen.'), 15, 512) ?>,
        thisJudoka: <?php echo json_encode(__('deze judoka'), 15, 512) ?>,
        moveBackToPreviousRound: <?php echo json_encode(__(':naam terugplaatsen naar vorige ronde?'), 15, 512) ?>,
        thisResetsSourceMatch: <?php echo json_encode(__('Dit reset de uitslag van de bron-wedstrijd.'), 15, 512) ?>,
    };
...
<div id="mat-interface" x-data="matInterface()" x-init="init()">
    ...
    <div class="mb-1" x-show="blokId && matId">
        <div class="flex items-center justify-between flex-wrap gap-x-4 gap-y-0.5">
            ...
            <div class="text-sm text-gray-600 flex items-center gap-2">
                ...
                <a href="#blok-mat-keuze" class="text-purple-600 hover:underline text-xs">(<?php echo e(__('wijzig')); ?>)</a>
                <?php if(!isset($isDeviceBound) || !$isDeviceBound): ?>
            ...
            <div class="flex items-center gap-3 text-xs">
                <span class="flex items-center gap-1">
                    ...
                    <span class="text-gray-600"><?php echo e(__('Speelt nu')); ?></span>
                </span>
                <span class="flex items-center gap-1">
                    <span class="w-3 h-3 rounded bg-yellow-400"></span>
                    <span class="text-gray-600"><?php echo e(__('Klaar staan')); ?></span>
                </span>
                <span class="flex items-center gap-1">
                    <span class="w-3 h-3 rounded bg-blue-400"></span>
                    <span class="text-gray-600"><?php echo e(__('Gereed maken')); ?></span>
                </span>
            </div>
        </div>
        <p class="text-xs text-gray-400 mt-0.5"><?php echo e(__('Klik op wedstrijdnummer om beurtkleur in te stellen')); ?></p>
    </div>
            ...
            <div :class="poule.type === 'eliminatie' ? 'bg-purple-700' : 'bg-green-700'" class="text-white px-3 py-1.5 flex justify-between items-center">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div x-show="poule.spreker_klaar" class="bg-white px-3 py-1 rounded text-sm font-bold" :class="poule.type === 'eliminatie' ? 'text-purple-700' : 'text-green-700'">
                        ✓ <?php echo e(__('Klaar om:')); ?> <span x-text="poule.spreker_klaar_tijd"></span>
                    </div>
                ...
                <div class="p-1" x-data="{ activeTab: poule.groep_filter || 'A' }">
                    ...
                    <div class="flex mb-1 border-b border-gray-200 justify-between">
                        <div class="flex">
                            <button x-show="!poule.groep_filter || poule.groep_filter === 'A'"
                                ...
                                <?php echo e(__('Groep A (Hoofdboom)')); ?> <span x-text="'(' + poule.judoka_count + ')'"></span>
                            </button>
                            <template x-if="heeftHerkansing(poule) && (!poule.groep_filter || poule.groep_filter === 'B')">
                                <button @click="activeTab = 'B'; $nextTick(() => { laadBracketHtml(poule.poule_id, 'B'); })"
                                        ...
                                        class="px-4 py-1 text-xs font-bold border-b-2 transition-colors">
                                    <?php echo e(__('Groep B (Herkansing)')); ?> <span x-text="'(' + (poule.eliminatie_type === 'ijf' ? Math.min(6, poule.judoka_count) : poule.judoka_count - 2) + ')'"></span>
                                </button>
                            </template>
                        </div>
                        ...
                        <div x-show="!isBracketLocked(poule) && activeTab !== 'B'" class="flex items-center gap-2 px-2">
                            ...
                            <div class="flex flex-wrap gap-1 min-w-[200px] max-w-[400px] min-h-[32px] border-2 border-dashed rounded-lg px-2 py-1 bg-orange-50 border-orange-400 bracket-drop bracket-swap"
                                ...
                                <span x-show="getSwapJudokas(poule.poule_id).length === 0" class="text-sm italic py-1 text-orange-400"><?php echo e(__('Sleep judoka hierheen')); ?></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-show="activeTab === 'A'">
                        <div class="flex justify-between items-center flex-wrap gap-1">
                            <div class="flex items-center gap-1">
                                ...
                                <span class="bracket-nav-controls" :id="'bracket-nav-' + poule.poule_id + '-A'">
                                    ...
                                    <span class="text-xs text-gray-400 bracket-nav-label"><?php echo e(__('Alle rondes')); ?></span>
                                    <button type="button" onclick="bracketNavigate(this.closest('[id^=bracket-nav]').id.split('-')[2], 1)"
                        ...
                        <div class="bracket-container overflow-auto pb-2" style="max-height: calc(100vh - 180px);"
                            ...
                            <div class="text-gray-400 text-sm py-4"><?php echo e(__('Bracket laden...')); ?></div>
                        </div>
                    </div>
                    ...
                    <table class="w-full text-sm border-collapse">
                        <thead>
                            <tr class="bg-gray-200 border-b-2 border-gray-400">
                                <th class="px-2 py-1 text-left font-bold text-gray-700 sticky left-0 bg-gray-200 min-w-[240px]"><?php echo e(__('Naam')); ?></th>
    ...
    <div x-show="poules.length === 0 && blokId && matId" class="bg-white rounded-lg shadow p-8 text-center text-gray-500">
        <?php echo e(__('Geen poules op deze mat in dit blok')); ?>

    </div>

    <!-- Blok/Mat keuze onderaan -->
    <div id="blok-mat-keuze" class="bg-gray-100 rounded-lg p-4 mt-8 border border-gray-300">
        <h3 class="text-sm font-bold text-gray-600 mb-3"><?php echo e(__('Blok & Mat selectie')); ?></h3>
        <div class="flex gap-4">
            <div class="w-40">
                <label class="block text-gray-600 text-sm mb-1"><?php echo e(__('Blok')); ?></label>
                <select x-model="blokId" @change="laadWedstrijden()" class="w-full border rounded px-3 py-2 text-sm">
                    ...
                    <option value="<?php echo e($blok->id); ?>"><?php echo e(__('Blok')); ?> <?php echo e($blok->nummer); ?>

                        <?php if($blok->weging_gesloten): ?> (<?php echo e(__('gesloten')); ?>) <?php endif; ?>
                    </option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </div>
            <div class="w-40">
                <label class="block text-gray-600 text-sm mb-1"><?php echo e(__('Mat')); ?></label>
                <select x-model="matId" @change="laadWedstrijden()" class="w-full border rounded px-3 py-2 text-sm">
                    <option value=""><?php echo e(__('Selecteer...')); ?></option>
                    <?php $__currentLoopData = $matten; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $mat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($mat->id); ?>">Mat <?php echo e($mat->nummer); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </div>
        </div>
...
<script>
// === TRANSLATION STRINGS ===
const __eliminatie = <?php echo json_encode(__('Eliminatie'), 15, 512) ?>;
const __poule = <?php echo json_encode(__('Poule'), 15, 512) ?>;
const __finale = <?php echo json_encode(__('Finale'), 15, 512) ?>;
const __brons = <?php echo json_encode(__('Brons'), 15, 512) ?>;

const __bracketVergrendeld = <?php echo json_encode(__('BRACKET VERGRENDELD'), 15, 512) ?>;
const __probeertTeVerwijderen = <?php echo json_encode(__('Je probeert :naam te verwijderen.'), 15, 512) ?>;
const __ditKanAlleenDoorAdmin = <?php echo json_encode(__('Dit kan alleen door de organisator of hoofdjury.'), 15, 512) ?>;
const __voerAdminWachtwoordIn = <?php echo json_encode(__('Voer het organisator wachtwoord of hoofdjury pincode in:'), 15, 512) ?>;
const __onjuistWachtwoord = <?php echo json_encode(__('Onjuist wachtwoord! Verwijdering geannuleerd.'), 15, 512) ?>;
...
window.promptWachtwoord = function(bericht) {
    return new Promise((resolve) => {
        ...
        overlay.innerHTML = `
            <div style="background:white;border-radius:8px;padding:24px;max-width:400px;width:90%;box-shadow:0 4px 20px rgba(0,0,0,0.3);">
                ...
                <div style="position:relative;">
                    <input type="password" id="ww-input" style="width:100%;padding:8px 12px;padding-right:40px;border:2px solid #d1d5db;border-radius:6px;font-size:16px;box-sizing:border-box;" placeholder="${__t.passwordOrPinPlaceholder}" autocomplete="off">
                    <button type="button" id="ww-eye" style="position:absolute;right:8px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;padding:4px;color:#6b7280;">
                        <svg id="ww-eye-closed" width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/></svg>
                        <svg id="ww-eye-open" width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="display:none;"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                    </button>
                </div>
        ...
        const eyeBtn = overlay.querySelector('#ww-eye');
        const eyeClosed = overlay.querySelector('#ww-eye-closed');
        const eyeOpen = overlay.querySelector('#ww-eye-open');
        eyeBtn.onclick = () => {
...
window.checkAdminWachtwoord = async function(wachtwoord) {
    try {
        const response = await csrfFetch(`<?php echo e($checkWachtwoordUrl); ?>`, {
            method: 'POST',
            headers: {
...
window.dropInSwap = async function(event, pouleId, isLocked = false) {
    ...
    if (data.wedstrijdId && data.positie) {
        try {
            const response = await csrfFetch(`<?php echo e($verwijderJudokaUrl); ?>`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
    ...
    if (isWinnaarDoorschuif) {
        ...
        if (isSeeding) {
            ...
            if (data.wedstrijdId && data.positie && !data.fromSwap) {
                await csrfFetch(`<?php echo e($verwijderJudokaUrl); ?>`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
        ...
        const response = await csrfFetch(`<?php echo e($plaatsJudokaUrl); ?>`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
...
window.dropOpMedaille = async function(event, finaleId, medaille, pouleId) {
    ...
    try {
        const response = await csrfFetch(`<?php echo e($finaleUitslagUrl); ?>`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
...
function matInterface() {
    ...
    const blokkenData = <?php echo json_encode($blokken->map(fn($b) => ['id' => $b->id, 'nummer' => $b->nummer]), 512) ?>;
    const mattenData = <?php echo json_encode($matten->map(fn($m) => ['id' => $m->id, 'nummer' => $m->nummer]), 512) ?>;
    const voorgeselecteerdBlok = blokNummer ? blokkenData.find(b => b.nummer == blokNummer) : null;
    const isDeviceBound = <?php echo e(isset($isDeviceBound) && $isDeviceBound ? 'true' : 'false'); ?>;
    const gebondenMatNummer = <?php echo e(isset($matNummer) && $matNummer ? $matNummer : 'null'); ?>;

    // LocalStorage key voor dit toernooi (include mat for device-bound)
    const storageKey = isDeviceBound && gebondenMatNummer
        ? 'mat_interface_<?php echo e($toernooi->id); ?>_mat_' + gebondenMatNummer
        : 'mat_interface_<?php echo e($toernooi->id); ?>';
    ...
    return {
        ...
        init() {
            ...
            } else if (this.blokId && <?php echo json_encode($matten->count(), 15, 512) ?> > 0) {
                // Fallback: eerste mat selecteren (of gebonden mat)
                this.matId = forcedMatId || '<?php echo e($matten->first()?->id); ?>';
                this.laadWedstrijden();
            }
        },
        ...
        async laadWedstrijden() {
            ...
            try {
                const apiUrl = `<?php echo e($wedstrijdenUrl); ?>`;
        ...
        async laadBracketHtml(pouleId, groep) {
            ...
            try {
                const response = await csrfFetch(`<?php echo e($bracketHtmlUrl); ?>`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
        ...
        async advanceByes(pouleId) {
            try {
                const response = await csrfFetch(`<?php echo e($advanceByesUrl); ?>`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
        ...
        async saveScore(wedstrijd, poule) {
            ...
            // Save to backend
            const uitslagResponse = await csrfFetch(`<?php echo e($uitslagUrl); ?>`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
        ...
        async markeerKlaar(poule) {
            try {
                const response = await csrfFetch(`<?php echo e($pouleKlaarUrl); ?>`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
        ...
        async maakBarrage(poule) {
            ...
            try {
                const response = await csrfFetch(`<?php echo e(route('toernooi.mat.barrage', $toernooi->routeParams())); ?>`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
        ...
        async setWedstrijdStatus(actieveId, volgendeId, gereedmakenId) {
            ...
            try {
                const url = `<?php echo e($huidigeWedstrijdUrl); ?>`;
...
<?php /**PATH /var/www/judotoernooi/staging/resources/views/pages/mat/partials/_content.blade.php ENDPATH**/ ?>