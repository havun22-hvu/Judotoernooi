...
<button id="pwa-settings-floating-btn"
        ...
        title="<?php echo e(__('Instellingen')); ?>"
        style="display: none;">
...
<div id="pwa-settings-modal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"
    ...
    <div class="bg-white rounded-lg shadow-xl w-72 text-gray-800 overflow-hidden">
        <div class="px-4 py-3 border-b flex justify-between items-center">
            <h2 class="font-bold text-gray-900"><?php echo e($config['name']); ?></h2>
        ...
        <div class="px-4 py-3 space-y-3 text-sm">
            <div class="flex justify-between items-center">
                <span class="text-gray-500"><?php echo e(__('Versie')); ?></span>
                <span class="font-medium">v<?php echo e($appVersion); ?></span>
            </div>
            <?php if($appVersionDate): ?>
            <div class="flex justify-between items-center">
                <span class="text-gray-500"><?php echo e(__('Update')); ?></span>
                <span class="text-gray-700"><?php echo e($appVersionDate); ?></span>
            </div>
            <?php endif; ?>
            <div class="flex justify-between items-center">
...
<div id="pwa-install-banner" class="hidden fixed bottom-0 left-0 right-0 bg-green-600 text-white p-4 z-50 safe-area-bottom">
    <div class="flex items-center justify-between max-w-lg mx-auto">
        <div class="flex items-center gap-3">
            <span class="text-2xl"><?php echo e($config['deviceIcon']); ?></span>
            <div>
                <p class="font-bold"><?php echo e(__('Installeer')); ?> <?php echo e($config['name']); ?></p>
                <p class="text-sm text-green-100"><?php echo e($config['device']); ?></p>
            </div>
        </div>
        <div class="flex gap-2">
            <button onclick="hidePwaInstallBanner()" class="px-3 py-1 text-green-200 hover:text-white"><?php echo e(__('Later')); ?></button>
            <button onclick="installPwa()" class="bg-white text-green-600 px-4 py-1 rounded font-medium"><?php echo e(__('Installeer')); ?></button>
        </div>
    </div>
</div>
...
<div id="pwa-update-banner" class="hidden fixed top-0 left-0 right-0 bg-orange-500 text-white p-3 z-50 safe-area-top">
    <div class="flex items-center justify-center max-w-lg mx-auto gap-2">
        ...
        <p class="font-medium"><?php echo e(__('Updaten... pagina herlaadt automatisch')); ?></p>
    </div>
</div>
...
<script>
    ...
    window.addEventListener('beforeinstallprompt', (e) => {
        ...
        setTimeout(() => {
            if (deferredPrompt && !localStorage.getItem('pwa-install-dismissed-<?php echo e($pwaApp); ?>')) {
    ...
    function hidePwaInstallBanner() {
        document.getElementById('pwa-install-banner').classList.add('hidden');
        localStorage.setItem('pwa-install-dismissed-<?php echo e($pwaApp); ?>', Date.now());
    }
    ...
    let newWorker = null;
    const APP_VERSION = '<?php echo e($appVersion); ?>';
...
<?php /**PATH /var/www/judotoernooi/staging/resources/views/partials/pwa-mobile.blade.php ENDPATH**/ ?>