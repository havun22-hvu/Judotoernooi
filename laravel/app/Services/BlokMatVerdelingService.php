...
use App\Services\BlokMatVerdeling\MatAssigner;
use App\Services\BlokMatVerdeling\ZaalOverzichtBuilder;
...
class BlokMatVerdelingService
{
    ...
    private MatAssigner $matAssigner;
    private ZaalOverzichtBuilder $zaalOverzichtBuilder;
    ...
    public function __construct(
        ...
        ?MatAssigner $matAssigner = null,
        ?ZaalOverzichtBuilder $zaalOverzichtBuilder = null
    ) {
        $this->scoreCalculator = $scoreCalculator;
        $this->capaciteitHelper = $capaciteitHelper;
        $this->plaatsingsHelper = $plaatsingsHelper;
        $this->categorieHelper = $categorieHelper;
        $this->matAssigner = $matAssigner ?? new MatAssigner();
        $this->zaalOverzichtBuilder = $zaalOverzichtBuilder ?? new ZaalOverzichtBuilder();
    }
     ...
     *
     * @see MatAssigner::verdeelOverMatten
     */
    public function verdeelOverMatten(Toernooi $toernooi): void
    {
        $this->matAssigner->verdeelOverMatten($toernooi);
    }

    /**
     * @see MatAssigner::getCategorieVolgorde
     */
    private function getCategorieVolgorde(?string $leeftijdsklasse): int
    {
        return $this->matAssigner->getCategorieVolgorde($leeftijdsklasse);
    }

    /**
     * @see MatAssigner::extractLeeftijdUitCategorieKey
     */
    private function extractLeeftijdUitCategorieKey(?string $categorieKey): int
    {
        return $this->matAssigner->extractLeeftijdUitCategorieKey($categorieKey);
    }

    /**
     * @see MatAssigner::bepaalZwaarGewichtGrens
     */
    private function bepaalZwaarGewichtGrens($poules): float
    {
        return $this->matAssigner->bepaalZwaarGewichtGrens($poules);
    }

    /**
     * @see MatAssigner::isPouleVoorDames
     */
    private function isPouleVoorDames(Poule $poule): bool
    {
        return $this->matAssigner->isPouleVoorDames($poule);
    }

    /**
     * @see MatAssigner::extractGewichtVoorSortering
     */
    private function extractGewichtVoorSortering(?string $gewichtsklasse): float
    {
        return $this->matAssigner->extractGewichtVoorSortering($gewichtsklasse);
    }

    /**
     * @see MatAssigner::fixKruisfinaleMatten
     */
    private function fixKruisfinaleMatten(Toernooi $toernooi): void
    {
        $this->matAssigner->fixKruisfinaleMatten($toernooi);
    }

    /**
     * @see MatAssigner::vindMinsteWedstrijdenMat
     */
    private function vindMinsteWedstrijdenMat(array $matIds, array $wedstrijdenPerMat): int
    {
        return $this->matAssigner->vindMinsteWedstrijdenMat($matIds, $wedstrijdenPerMat);
    }
     ...
     *
     * @see ZaalOverzichtBuilder::build
     */
    public function getZaalOverzicht(Toernooi $toernooi): array
    {
        return $this->zaalOverzichtBuilder->build($toernooi);
    }
}
