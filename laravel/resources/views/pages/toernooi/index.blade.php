...
@section('content')
<div x-data="tabSwitch" data-initial-tab="overzicht" class="w-full max-w-7xl">

{{-- Header --}}
<div class="flex justify-between items-center mb-6">
    <div>
        <h1 class="text-3xl font-bold text-gray-800">{{ __('Havun Admin Dashboard') }}</h1>
        <p class="text-gray-500 mt-1 text-sm">{{ __('Overzicht van alle klanten (organisatoren) en hun toernooien') }}</p>
    </div>
...
<nav class="flex gap-1 mb-6 border-b border-gray-200">
    ...
    <button @click="switchTo('systeem')" :class="tab === 'systeem' ? 'border-blue-600 text-blue-600 font-semibold' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'" class="px-4 py-2.5 border-b-2 text-sm transition-colors">
        ...
        @if($afCount > 0)
            <span class="ml-1 bg-red-100 text-red-700 text-xs font-bold px-1.5 py-0.5 rounded-full">{{ $afCount }}</span>
        @endif
    </button>
</nav>
...
<div x-show="tab === 'overzicht'" x-cloak class="w-full">
    ...
    {{-- Vandaag/Binnenkort + Klant Gezondheid --}}
    ...
    @endphp
    @if($actieveOrganisatoren->count() > 0)
    <div class="bg-white rounded-lg shadow overflow-hidden mb-6">
        <div class="bg-green-50 px-6 py-3 border-b">
            <h2 class="text-lg font-bold text-green-800">{{ __('Nu actief') }} ({{ __('laatste 24 uur') }})</h2>
        </div>
        <div class="divide-y divide-gray-100">
            @foreach($actieveOrganisatoren as $org)
            @php
                $laatsteToernooi = $org->toernooien->whereNull('afgesloten_at')->sortByDesc('updated_at')->first();
            @endphp
            <div class="px-6 py-3 flex items-center justify-between hover:bg-gray-50">
                <div class="flex items-center gap-3">
                    <span class="w-2 h-2 bg-green-500 rounded-full {{ $org->laatste_login->diffInMinutes() < 30 ? 'animate-pulse' : '' }}"></span>
                    <div>
                        <span class="font-medium text-sm">{{ $org->naam }}</span>
                        @if($laatsteToernooi)
                            <span class="text-gray-400 mx-1">&rarr;</span>
                            <span class="text-sm text-gray-600">{{ $laatsteToernooi->naam }}</span>
                            @if($laatsteToernooi->weegkaarten_gemaakt_op)
                                <span class="ml-1 px-1.5 py-0.5 bg-green-100 text-green-700 rounded text-xs">{{ __('Wedstrijddag') }}</span>
                            @else
                                <span class="ml-1 px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded text-xs">{{ __('Voorbereiding') }}</span>
                            @endif
                        @endif
                    </div>
                </div>
...
<div x-show="tab === 'klanten'" x-cloak class="w-full">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.klanten') }}" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 text-sm">
            {{ __('Klantenbeheer') }}
        </a>
    </div>
            ...
            <div class="flex justify-between items-start">
                <div>
                    <div class="flex items-center gap-3">
                        ...
                        <a href="{{ route('organisator.dashboard', ['organisator' => $organisator->slug]) }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">&rarr; {{ __('Open dashboard') }}</a>
                    </div>
                    ...
                    <td class="px-6 py-3 whitespace-nowrap text-center">
                        @if($toernooi->isPaidTier())
                            <div class="inline-flex flex-col items-center">
                                <span class="px-2 py-1 bg-green-100 text-green-700 rounded text-xs font-medium">&euro;{{ $toernooi->toernooiBetaling?->bedrag ?? '?' }}</span>
    ...
    <div class="bg-white rounded-lg shadow mb-6 overflow-hidden mt-8">
        <div class="bg-orange-50 px-6 py-4 border-b">
            <h2 class="text-lg font-bold text-orange-800">{{ __('Toernooien zonder organisator') }} ({{ $toernooienZonderOrganisator->count() }})</h2>
...
<div x-show="tab === 'systeem'" x-cloak class="w-full">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.autofix') }}" class="bg-gray-600 text-white px-4 py-2 rounded hover:bg-gray-700 text-sm">
            {{ __('AutoFix openen') }} &rarr;
        </a>
    </div>
