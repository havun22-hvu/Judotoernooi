...
@section('title', __('Upgrade Toernooi') . ' - ' . $toernooi->naam)

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <a href="{{ route('toernooi.show', $toernooi->routeParams()) }}" class="text-blue-600 hover:underline">&larr; {{ __('Terug naar toernooi') }}</a>
    </div>

    <h1 class="text-3xl font-bold text-gray-800 mb-2">
        {{ ($isReUpgrade ?? false) ? __('Meer judoka\'s nodig?') : __('Upgrade naar Betaald') }}
    </h1>
    ...
    <div class="{{ ($isReUpgrade ?? false) ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-200' }} border rounded-lg p-6 mb-8">
        <h2 class="text-lg font-semibold {{ ($isReUpgrade ?? false) ? 'text-green-800' : 'text-blue-800' }} mb-4">
            {{ __('Huidige Status') }}: {{ ($isReUpgrade ?? false) ? __('Betaald') . ' (max ' . $status['max_judokas'] . ' ' . __("judoka's") . ')' : __('Gratis Tier') }}
        </h2>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
                <span class="text-gray-600">{{ __("Judoka's") }}</span>
    ...
    <div class="flex items-center mb-8">
        <div class="flex items-center">
            ...
            <span class="ml-2 font-medium {{ $kycCompleet ? 'text-green-600' : 'text-blue-600' }}">{{ __('Facturatiegegevens') }}</span>
        </div>
        ...
        <div class="flex items-center">
            ...
            <span class="ml-2 font-medium {{ $kycCompleet ? 'text-blue-600' : 'text-gray-400' }}">{{ __('Staffel kiezen & betalen') }}</span>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-8">
        <h2 class="text-xl font-bold text-gray-800 mb-4">{{ __('Stap 1: Facturatiegegevens') }}</h2>
        <p class="text-gray-600 mb-6">{{ __('Vul je organisatiegegevens in. Deze worden gebruikt voor de factuur.') }}</p>
        ...
        <form action="{{ route('toernooi.upgrade.kyc', $toernooi->routeParams()) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    <label for="organisatie_naam" class="block text-gray-700 font-medium mb-1">{{ __('Organisatie/Verenigingsnaam') }} *</label>
                    <input type="text" name="organisatie_naam" id="organisatie_naam"
                           value="{{ old('organisatie_naam', $organisator->organisatie_naam) }}"
                           placeholder="{{ __('Bijv. Judoschool Cees Veen') }}"
                ...
                <div>
                    <label for="kvk_nummer" class="block text-gray-700 font-medium mb-1">{{ __('KvK-nummer') }}</label>
                ...
                <div>
                    <label for="btw_nummer" class="block text-gray-700 font-medium mb-1">{{ __('BTW-nummer') }}</label>
                ...
                <div class="md:col-span-2">
                    <label for="straat" class="block text-gray-700 font-medium mb-1">{{ __('Straat en huisnummer') }} *</label>
                    <input type="text" name="straat" id="straat"
                           value="{{ old('straat', $organisator->straat) }}"
                           placeholder="{{ __('Hoofdstraat 123') }}"
                ...
                <div>
                    <label for="postcode" class="block text-gray-700 font-medium mb-1">{{ __('Postcode') }} *</label>
                ...
                <div>
                    <label for="plaats" class="block text-gray-700 font-medium mb-1">{{ __('Plaats') }} *</label>
                ...
                <div>
                    <label for="land" class="block text-gray-700 font-medium mb-1">{{ __('Land') }} *</label>
                ...
                <div>
                    <label for="contactpersoon" class="block text-gray-700 font-medium mb-1">{{ __('Contactpersoon') }} *</label>
                ...
                <div>
                    <label for="telefoon" class="block text-gray-700 font-medium mb-1">{{ __('Telefoonnummer') }}</label>
                ...
                <div>
                    <label for="factuur_email" class="block text-gray-700 font-medium mb-1">{{ __('E-mail voor factuur') }} *</label>
    ...
    <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-8">
        <div class="flex justify-between items-start">
            <div>
                <h3 class="font-semibold text-green-800 mb-2">{{ __('Facturatiegegevens') }}</h3>
                <div class="text-sm text-green-700">
                    <p><strong>{{ $organisator->organisatie_naam }}</strong></p>
                    <p>{{ $organisator->straat }}</p>
                    <p>{{ $organisator->postcode }} {{ $organisator->plaats }}</p>
                    @if($organisator->kvk_nummer)<p>{{ __('KvK') }}: {{ $organisator->kvk_nummer }}</p>@endif
                    <p>{{ $organisator->factuur_email }}</p>
                </div>
            </div>
            <a href="{{ route('toernooi.upgrade', $toernooi->routeParams()) }}?edit=1" class="text-green-600 hover:underline text-sm">{{ __('Wijzigen') }}</a>
        </div>
    </div>

    {{-- Upgrade options --}}
    <h2 class="text-2xl font-bold text-gray-800 mb-4">{{ __('Stap 2: Kies je aantal judoka\'s') }}</h2>

    @if(count($upgradeOptions) === 0)
        <div class="bg-gray-100 rounded-lg p-6 text-center">
            <p class="text-gray-600">{{ __('Er zijn geen upgrade opties beschikbaar. Het maximum aantal judoka\'s is bereikt.') }}</p>
        </div>
    @else
        <div class="bg-white rounded-lg shadow p-6 mb-8">
            {{-- Simple selector --}}
            <div class="flex flex-col md:flex-row md:items-end gap-6">
                <div class="flex-1">
                    <label for="tier-select" class="block text-gray-700 font-medium mb-2">{{ __('Hoeveel judoka\'s heb je nodig?') }}</label>
                ...
                <div id="price-display" class="hidden">
                    <p class="text-gray-600 text-sm mb-1">{{ __('Eenmalige kosten') }}</p>
            ...
            <div id="features-section" class="hidden mt-6 pt-6 border-t">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                    <div class="flex items-center text-gray-600">
                        ...
                        {{ __('Tot') }} <span id="max-judokas" class="font-semibold mx-1">0</span> {{ __("judoka's") }}
                    </div>
                    <div class="flex items-center text-gray-600">
                        <svg class="w-5 h-5 text-green-500 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                    ...
                    <div class="flex items-center text-gray-600">
                        ...
                        {{ __('Danpunten registreren (JBN export)') }}
                    </div>
                </div>
            </div>
