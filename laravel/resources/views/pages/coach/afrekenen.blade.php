...
<head>
    ...
    <title>{{ __('Afrekenen') }} - {{ $club->naam }}</title>
...
<body class="bg-gray-100 min-h-screen">
    <div class="max-w-2xl mx-auto py-8 px-4">
        ...
        <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
            <h3 class="font-medium text-green-800 mb-2">{{ __('Al betaald (:count judoka\'s)', ['count' => $reedsBetaald->count()]) }}</h3>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden mb-6">
            <div class="px-6 py-4 border-b bg-gray-50">
                <h2 class="text-xl font-bold text-gray-800">{{ __('Te betalen (:count judoka\'s)', ['count' => $klaarVoorBetaling->count()]) }}</h2>
            </div>
            <div class="divide-y">
        ...
        <div class="bg-white rounded-lg shadow p-8 text-center">
            <p class="text-gray-600 mb-4">{{ __('Geen judoka\'s om af te rekenen.') }}</p>
            <a href="{{ route('coach.portal.judokas', ['organisator' => $organisator, 'toernooi' => $toernooiSlug, 'code' => $code]) }}"
               class="text-blue-600 hover:text-blue-800">
                {{ __('Terug naar judoka\'s') }}
            </a>
        </div>
        @endif
    </div>
