                ...
                <div class="px-3 py-2 border-b {{ $isEliminatie ? 'bg-orange-100' : ($isKruisfinale ? 'bg-purple-100' : ($isProbleem ? 'bg-red-100' : 'bg-blue-100')) }}">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex items-center gap-2">
                            @if($isEliminatie)
                                @if($poule->mat_id)
                                <a href="{{ route('toernooi.poule.eliminatie', [$toernooi, $poule]) }}" class="bg-white text-gray-800 border border-gray-300 hover:bg-gray-50 text-xs font-medium px-2 py-1 rounded" title="Bekijk/genereer schema">Schema →</a>
                                @else
