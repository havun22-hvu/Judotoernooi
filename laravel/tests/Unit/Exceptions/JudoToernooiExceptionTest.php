...
{
    public function test_it_creates_base_exception_with_user_message(): void
    {
        ...
        // Context is enriched with exception_class + technical_message since
        // 2026-02 — assert the user-supplied keys survive, not exact equality.
        $this->assertSame('value', $exception->getContext()['key']);
    }

    public function test_it_uses_default_user_message_when_not_provided(): void
    {
        // API change since 2026-02: when userMessage is omitted, the class
        // now returns a generic fallback ('Er ging iets mis...') instead of
        // echoing the technical message. This is intentional for safety —
        // technical details should not leak into the user-facing layer.
        $exception = new JudoToernooiException('Error message');

        $this->assertEquals('Er ging iets mis. Probeer het opnieuw.', $exception->getUserMessage());
